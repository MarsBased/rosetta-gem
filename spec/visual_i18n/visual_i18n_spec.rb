...
RSpec.describe VisualI18n do
  describe '#add_phrase' do
    it 'adds the phrase when correct arguments' do
      VisualI18n.add_phrase(code: 'foo.bar', text: 'Hello World!')
    ...
    it 'requires a code argument' do
      expect do
    ...
    it 'requires a text argument' do
      expect do
        VisualI18n.add_phrase(code: 'foo.bar')
      end.to raise_error(ArgumentError)
    end
  end
  ...
  describe '#store_phrases?' do
    ...
    it 'does not store phrases when the collection is not initialized' do
